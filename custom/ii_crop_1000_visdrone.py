...
import numpy as np
import cv2
from Constants import raw_val_img_dir,raw_val_ann_dir,temp_txt_dir,crop_1000_img_dir,crop_1000_xml_dir
    ...
    if len(boxes_all) > 0:
        ...
        if len(idx) > 0:
            ...
            if img_new.shape[0] > 5 and img_new.shape[1] > 5:
                img = os.path.join(crop_1000_img_dir, name_new + '.jpg')
                cv2.imwrite(img, img_new)
        else:
            print("empty dts, name:{}".format(name_new))

if __name__ == '__main__':
    print("box_dir:",temp_txt_dir)
    f_num=0
    clip_num = 0
    for i in [i for i in os.listdir(temp_txt_dir) if i[-4:] == '.txt']:
#         print(i)
        f_num +=1
        with open(os.path.join(temp_txt_dir, i), 'r', encoding='utf8') as f:
            ...
            for line in lines:
                ...
                x1, y1, x2, y2 = int(x1), int(y1), int(float(x2)), int(y2)
                name_old = name_new[:6]
            ...
            for name_old, datas in drawer.items():
                ...
                for data_new in datas:
                    name_new, x1, y1, x2, y2 = data_new
                    path_new = os.path.join(crop_1000_img_dir, name_new + '.jpg')
                    path_new_xml = os.path.join(crop_1000_xml_dir,name_new + '.xml')
